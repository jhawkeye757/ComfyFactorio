...
local quality_init = require("quality_init") -- Ensure quality_init is loaded
...
-- Save the original function in case you need to call it later
local original_create_crash_site = crash_site.create_crash_site

local function get_quality()
    local current_quality = global.NORMAL_QUALITY.next
    local cumulative_probability = .6
    local random_value = math.random()

    while random_value <= cumulative_probability do
        if current_quality.next ~= nil then
            current_quality = current_quality.next
            cumulative_probability = cumulative_probability * .6
        else
            break
        end
    end
...
crash_site.create_crash_site = function(surface, position, ship_items, part_items, ship_parts)
	local main_ship_name = "crash-site-spaceship"
	local main_ship_flame_count = 4
	local main_ship_explosion_count = 10
...
-- Module interface for `event_handler`
local quality_crashsite = {}


return quality_crashsite