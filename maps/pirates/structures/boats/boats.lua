...
function Public.currentdestination_move_boat_natural()
	...
	if
		(destination and destination.dynamic_data and destination.dynamic_data.timer)
		and (destination.dynamic_data.timer < 1)
	then
		return
	end
...
function Public.upgrade_chests(boat, new_chest)
	...
	local ps = Common.entity_positions_from_blueprint(
		scope.Data.entities.static.bp_str,
		Math.vector_sum(boat.position, scope.Data.upgrade_chests.pos)
	)
...
function Public.place_boat(boat, floor_tile, place_entities_bool, correct_tiles, flipped)
	...
	if place_entities_bool then
		for etype, entitydata in pairs(scope.Data.entities) do
			...
			if entitydata.pos then
				entities_pos =
					{ x = boat.position.x + flipped_sign * entitydata.pos.x, y = boat.position.y + entitydata.pos.y }
			else
				entities_pos = { x = boat.position.x, y = boat.position.y }
			end
			local entities =
				Common.build_from_blueprint(entitydata.bp_str, surface, entities_pos, boat.force_name, flipped)
		...
		if scope.Data.EEIs then
			...
			for _, p in pairs(scope.Data.EEIs) do
				...
				if e and e.valid then
					...
					e.operable = false
					e.electric_buffer_size = boat.EEIelectric_buffer_size
						or Balance.starting_boatEEIelectric_buffer_size_MJ() * 1000000
					e.power_production = boat.EEIpower_production
						or Balance.starting_boatEEIpower_production_MW() * 1000000 / 60
					e.power_usage = 0
		...
		if scope.Data.upstairs_fluid_storages then
			if not boat.upstairs_fluid_storages then
		...
		if scope.Data.cannons then
			for _, p in pairs(scope.Data.cannons) do
				...
				if e and e.valid then
					...
					if not boat.cannons_temporary_reference then
						boat.cannons_temporary_reference = {}
					end
		...
		if scope.Data.cabin_car then
			local car_pos =
				{ x = boat.position.x + scope.Data.cabin_car.x, y = boat.position.y + scope.Data.cabin_car.y }
			local e = surface.create_entity({
				name = 'car',
				position = car_pos,
				force = boat.force_name,
		...
		if scope.Data.steering_boxes then
			if not boat.decksteeringchests then
			...
			for _, p in pairs(scope.Data.steering_boxes or {}) do
				...
				local ee = surface.create_entity({
					name = 'blue-chest',
					position = box_pos,
					force = boat.force_name,
					create_build_effect_smoke = false,
		...
		if scope.Data.loco_pos then
			Common.build_small_loco(
				surface,
				{ x = boat.position.x + scope.Data.loco_pos.x, y = boat.position.y + scope.Data.loco_pos.y },
				boat.force_name,
				{ 255, 106, 52 }
			)
		end
		...
		if scope.Data.deck_whitebelts_lrtp_order then
			if not boat.deck_whitebelts then
	...
	if scope.Data.market_pos then
		local p = {
			x = boat.position.x + flipped_sign * scope.Data.market_pos.x,
			y = boat.position.y + flipped_sign * scope.Data.market_pos.y,
		}
		local e = surface.create_entity({
			name = 'market',
			position = p,
...
function Public.put_deck_whitebelts_in_standard_order(boat)
	if boat and boat.deck_whitebelts and #boat.deck_whitebelts > 0 then
		...
		table.sort(i1, function(a, b)
			return boat.deck_whitebelts[a].position.y < boat.deck_whitebelts[b].position.y
				or (
					boat.deck_whitebelts[a].position.y == boat.deck_whitebelts[b].position.y
					and boat.deck_whitebelts[a].position.x < boat.deck_whitebelts[b].position.x
				)
		end) --true if a should be to the left of b
...
function Public.get_players_on_gate_tiles(boat, boatposition_override)
	...
	for _, relative_area in pairs(scope.Data.right_gate_tile_areas) do
		local area = {
			{
				position.x + relative_area[1][1] - Public.small_distance,
				position.y + relative_area[1][2] - Public.small_distance,
			},
			{
				position.x + relative_area[2][1] + Public.small_distance,
				position.y + relative_area[2][2] + Public.small_distance,
			},
		}
...
function Public.on_boat(boat, pos)
	...
	for _, relative_area in pairs(Public[boat.type].Data.tile_areas) do
		local area = {
			...
			{ boat.position.x + relative_area[2][1], boat.position.y + relative_area[2][2] },
		}
...
local function process_entity_on_boat_unteleportable(
	memory,
	boat,
	newsurface,
	vector,
	players_just_offside,
	oldsurface_name,
	newsurface_name,
	e,
	name
	...
	if ee and ee.valid then
		...
		if eee and eee.valid then
			e.destroy()
			ee = eee.clone({
				position = { p.x + vector.x, p.y + vector.y },
				surface = newsurface,
				force = f,
				create_build_effect_smoke = false,
			})
	...
	if ee and ee.valid then
		...
		if name == 'artillery-turret' then
			...
			if oldsurface_name == newsurface_name then -- push players
				local area1 =
					{ { ee.position.x - 1.5, ee.position.y - 1.5 }, { ee.position.x + 1.5, ee.position.y + 1.5 } }
				local area2 = {
					{ ee.position.x - vector.x - 1.5, ee.position.y - vector.y - 1.5 },
					{ ee.position.x - vector.x + 1.5, ee.position.y - vector.y + 1.5 },
				}
				...
				for _, char in pairs(players_just_offside) do
					if
						Utils.contains(intersectingcharacters, char) or Utils.contains(intersectingcharacters2, char)
					then
						char.teleport(vector.x, vector.y)
						teleportedbool = true
					end
					if teleportedbool and char and char.valid then --did I push you into water?
						local nearbytiles = {
							newsurface.get_tile(char.position.x - 1, char.position.y - 1),
							newsurface.get_tile(char.position.x - 1, char.position.y),
							newsurface.get_tile(char.position.x - 1, char.position.y + 1),
							newsurface.get_tile(char.position.x, char.position.y - 1),
							newsurface.get_tile(char.position.x, char.position.y),
							newsurface.get_tile(char.position.x, char.position.y + 1),
							newsurface.get_tile(char.position.x + 1, char.position.y - 1),
							newsurface.get_tile(char.position.x + 1, char.position.y),
							newsurface.get_tile(char.position.x + 1, char.position.y + 1),
						...
						for _, t in pairs(nearbytiles) do
							if Utils.contains(CoreData.water_tile_names, t.name) then
						...
						if watercount > 5 then
							...
							Common.notify_force(
								force,
								{ 'pirates.death_pushed_into_water_by_cannon', name2 },
...
local function process_entity_on_boat_teleportable(
	memory,
	boat,
	newsurface,
	newposition,
	...
	for _, v in pairs(wire_connnectors or {}) do
		...
		for _, c in pairs(connections or {}) do
			...
			if c2 and c2.valid and c2.owner and c2.owner.valid and c2.owner.position then
				...
				wire_connections_matrix[e2p.x][e2p.y][#wire_connections_matrix[e2p.x][e2p.y] + 1] =
					{ name = e.name, pos = p, wire = wire, wire_connector_id = wire_connector_id } --flip since we will read these backwards
			end
		end
	end
	...
	if
		not (
			e.name == 'car'
			or e.name == 'tank'
			or e.name == 'spidertron'
			or string.sub(e.name, 1, 14) == 'spidertron-leg'
		)
	then
		e.destroy()
	end
...
local function teleport_handle_wake_tiles(
	boat,
	dummyboat,
	newsurface_name,
	oldsurface_name,
	...
	if oldsurface_name == newsurface_name then
		...
		for i = 1, vectorlength, 1 do
			local adjustednewposition =
				{ x = newposition.x - (i - 1) * vectordirection.x, y = newposition.y - (i - 1) * vectordirection.y }
			for _, area in pairs(wakeareas) do
				for _, p in pairs(Common.central_positions_within_area(area, adjustednewposition)) do
					...
					if
						static_params
						and static_params.deepwater_xposition
						and (p.x <= static_params.deepwater_xposition - 0.5)
					then
						if t == 'water' then
							t = 'deepwater'
						else
					...
					if friendlyboat_bool and boat.state == enum_state.RETREATING and vector.x < 0 then --in this case we need to place some landing tiles, as the cannon juts out
						if
							(p.x >= boat.dockedposition.x + scope.Data.leftmost_gate_position)
							and (p.y <= scope.Data.upmost_gate_position or p.y >= scope.Data.downmost_gate_position)
						then
					...
					if Math.distance(boat.position, player.character.position) < 50 then
						...
						if tile.valid then
							if Utils.contains(CoreData.water_tile_names, tile.name) then
								local new_pos = oldsurface.find_non_colliding_position(
									'character',
...
local function teleport_prepare_new_tiles(
	boat,
	new_floor_tile,
	oldposition,
	oldsurface_name,
	...
	if oldsurface_name == newsurface_name then
		...
		local newtiles = {}
		for i = 1, vectorlength, 1 do
			local adjustedoldposition =
				{ x = oldposition.x + (i - 1) * vectordirection.x, y = oldposition.y + (i - 1) * vectordirection.y }
...
function Public.reassign_fluid_storage_references(boat, newsurface, newposition)
	...
	if scope.Data.upstairs_fluid_storages then
		for i = 1, #scope.Data.upstairs_fluid_storages do
			local storages = newsurface.find_entities_filtered({
				name = 'storage-tank',
				position = Utils.psum({ newposition, scope.Data.upstairs_fluid_storages[i] }),
				radius = 1.5,
			})
			if #storages == 1 then
...
local function process_saved_rails_and_wagons(saved_rails, saved_wagons, first_rail_found_p, vector, newsurface)
	...
	for _, ee in ipairs(saved_rails) do
		if ee and ee.valid then
			...
			ee.clone({
				position = { p.x + first_rail_found_p.x + vector.x, p.y + first_rail_found_p.y + vector.y },
				surface = newsurface,
				force = f,
				create_build_effect_smoke = false,
			})
...
function Public.teleport_boat(boat, newsurface_name, newposition, new_floor_tile, old_water_tile)
	...
	if friendlyboat_bool then
		if oldsurface_name == newsurface_name then
			...
			memory.spawnpoint =
				{ x = scope.Data.spawn_point.x + boat.position.x, y = scope.Data.spawn_point.y + boat.position.y }
		end
	...
	if oldsurface_name == newsurface_name then
		sorting_f = function(a, b)
			...
			if vector.x > 0 then
				return (
					(a.position.x > b.position.x) or ((a.position.x == b.position.x) and (a.position.y > b.position.y))
				)
			elseif vector.x < 0 then
				return (
					(a.position.x < b.position.x) or ((a.position.x == b.position.x) and (a.position.y > b.position.y))
			...
			if vector.y > 0 then
				return (
					(a.position.y > b.position.y) or ((a.position.y == b.position.y) and (a.position.x > b.position.x))
				)
			elseif vector.y < 0 then
				return (
					(a.position.y < b.position.y) or ((a.position.y == b.position.y) and (a.position.x > b.position.x))
	...
	Common.ensure_chunks_at(
		game.surfaces[newsurface_name],
		{ x = newposition.x - scope.Data.width, y = newposition.y },
		chunkloadradius
	)

	teleport_prepare_new_tiles(
		boat,
		new_floor_tile,
		oldposition,
		oldsurface_name,
	...
	for _, relative_area in pairs(scope.Data.tile_areas) do
		local area = {
			{ oldposition.x + relative_area[1][1], oldposition.y + relative_area[1][2] },
			{ oldposition.x + relative_area[2][1], oldposition.y + relative_area[2][2] },
		}
	...
	for i = 1, #entities_on_boat do
		...
		then
			...
			local ee = e.clone({
				position = { x = p.x - first_rail_found_p.x, y = p.y - first_rail_found_p.y },
				surface = game.surfaces['piratedev1'],
				force = f,
				create_build_effect_smoke = false,
			})
			if ee and ee.valid then
	...
	if first_rail_found_p then
		for i = 1, #entities_on_boat do
			...
			if
				e
				and e.valid
				and (e.name == 'cargo-wagon' or e.name == 'locomotive' or (e.name == 'entity-ghost' and (e.ghost_name == 'cargo-wagon' or e.ghost_name == 'locomotive')))
				and (not Utils.contains(unique_entities_list, e))
			then
				unique_entities_list[#unique_entities_list + 1] = e
				local p, f = e.position, e.force
	...
	for i = 1, #entities_on_boat do
		...
		if
			...
			and (
				e.name == 'cargo-wagon'
				or e.name == 'locomotive'
				or (e.name == 'entity-ghost' and (e.ghost_name == 'cargo-wagon' or e.name == 'locomotive'))
			)
		then
			e.destroy()
		end
...
function Public.is_boat_at_sea()
	...
	if
		...
		and boat.state
		and (
			boat.state == Public.enum_state.ATSEA_SAILING
			or boat.state == Public.enum_state.ATSEA_WAITING_TO_SAIL
			or boat.state == Public.enum_state.ATSEA_VICTORIOUS
			or boat.state == Public.enum_state.ATSEA_LOADING_MAP
		)
	then
		return true
	else
...
function Public.clear_fluid_from_ship_tanks(idx)
	...
	if
		boat.upstairs_fluid_storages
		and boat.upstairs_fluid_storages[idx]
		and boat.upstairs_fluid_storages[idx].valid
	then
	...
	if boat.downstairs_fluid_storages then
		for j = 1, memory.hold_surface_count do
			if
				boat.downstairs_fluid_storages[j]
				and boat.downstairs_fluid_storages[j][idx]
				and boat.downstairs_fluid_storages[j][idx].valid
			then
